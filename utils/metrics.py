...
def compute_metrics_test(gt, pred,  thresh, competition=True,n_classes=10):
    """
    Computes accuracy, precision, recall and F1-score from prediction scores.
    Args:
        gt: Pytorch tensor on GPU, shape = [n_samples, n_classes]
...
#     pdb.set_trace()
    ...
    """for i, cls in enumerate(indexes):
        try:
            #AUROCs.append(roc_auc_score(gt_np[:, i], pred_np[:, i],multi_class='ovr'))
            AUROCs.append(roc_auc_score(gt_np[:, i], pred_np[:, i], multi_class='ovr'))
        except ValueError as error:
            print('Error in computing accuracy for {}.\n Error msg:{}'.format(i, error))
            AUROCs.append(0)
        ...
        try:
            Senss.append(sensitivity_score(gt_np[:, i], (pred_np[:, i]>=THRESH),average='macro'))
        except ValueError:
            print('Error in computing precision for {}.'.format(i))
            Senss.append(0)
        ...
        try:
            Specs.append(specificity_score(gt_np[:, i], (pred_np[:, i]>=THRESH),average='macro'))
        except ValueError:
            print('Error in computing F1-score for {}.'.format(i))
            Specs.append(0)

        try:
            Pre.append(precision_score(gt_np[:, i], (pred_np[:, i]>=THRESH),average='macro'))
        except ValueError:
            print('Error in computing F1-score for {}.'.format(i))
            Pre.append(0)
    
        try:
            F1.append(f1_score(gt_np[:, i], (pred_np[:, i]>=THRESH),average='macro'))
        except ValueError:
            print('Error in computing F1-score for {}.'.format(i))
            F1.append(0)"""
    AUROCs=roc_auc_score(gt_np, pred_np, multi_class='ovr')
    Accus=accuracy_score(gt_np, np.argmax(pred_np,axis=1))
    matrix = compute_pred_matrix(gt_np, np.argmax(pred_np, axis=1),n_classes)
    gt_np_th = gt_np[np.max(pred_np, axis=1) > 0.95]
    pred_np_th = pred_np[np.max(pred_np, axis=1) > 0.95]
    matrix_th = compute_pred_matrix(gt_np_th, np.argmax(pred_np_th, axis=1),n_classes)
